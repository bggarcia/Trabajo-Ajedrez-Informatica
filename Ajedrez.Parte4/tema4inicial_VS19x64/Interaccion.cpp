...
/*void Interaccion::rebote1(Figura& f, ListaTableros lt) {
	float xmin = (lt.cas1.limite1.x + lt.cas1.limite2.x) / 2.0;
	float xmax = (lt.cas2.limite1.x + lt.cas2.limite2.x) / 2.0;
	float ymin = (lt.cas1.limite1.y + lt.cas1.limite2.y) / 2.0;
	float ymax = (lt.cas4.limite1.y + lt.cas4.limite2.y) / 2.0;

	if (f.posicion.x > xmax)
		f.posicion.x = xmax;
	if (f.posicion.x < xmin)
		f.posicion.x = xmin;
	//
	if (f.posicion.y > ymax)
		f.posicion.y = ymax;
	if (f.posicion.y < ymin)
		f.posicion.y = ymin;
}*/
//
